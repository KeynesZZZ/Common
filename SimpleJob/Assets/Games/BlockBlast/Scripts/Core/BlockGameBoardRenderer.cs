...
namespace SimpleBoard
{
    public class BlockGameBoardRenderer : MonoBehaviour, IUnityGameBoardRenderer, IGameBoardDataProvider<IBlockGridSlot>
    {
        ...
        [Space] [SerializeField] private float _tileSize = 0.8f;

        [Space] [SerializeField] private TileModel[] _gridTiles;

        private IGridTile[,] _gridSlotTiles;
        private IBlockGridSlot[,] _gameBoardSlots;
        ...
        public IBlockGridSlot[,] GetGameBoardSlots(int level)
        {
            ...
            for (var rowIndex = 0; rowIndex < _rowCount; rowIndex++)
            {
                for (var columnIndex = 0; columnIndex < _columnCount; columnIndex++)
                {
                    ...
                    _gameBoardSlots[rowIndex, columnIndex] =
                        new BlockGridSlot(gridTile, new GridPosition(rowIndex, columnIndex));
                }
            }
        }
