...
using Match3Game.SpecialItemDetectors;
using UnityEngine;
using Match3.Interfaces;
using Match3.Core;
using Match3Game.FillStrategies;

namespace Match3Game
{
    public class AppContext : IAppContext
    {
        private CanvasInputSystem _inputSystem;
        private UnityGameBoardRenderer _gameBoardRenderer;

     
        public void Construct(GameObject itemPrefab)
        {

            GameObject gameBoardObj = GameObject.Find("GameBoard");
            UnityGameBoardRenderer unityGameBoardRenderer = gameBoardObj.GetComponent<UnityGameBoardRenderer>();
            GameObject gameCanvasObj = GameObject.Find("GameCanvas");
            CanvasInputSystem canvasInputSystem = gameCanvasObj.GetComponent<CanvasInputSystem>();

            _inputSystem = canvasInputSystem;
            _gameBoardRenderer = unityGameBoardRenderer;
            _registeredTypes = new Dictionary<Type, object>();

            RegisterInstance<IInputSystem>(_inputSystem);
            // RegisterInstance<IGameUiCanvas>(gameUiCanvas);
