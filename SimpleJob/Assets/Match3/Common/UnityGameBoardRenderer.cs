...
namespace Match3Game
{
    public class UnityGameBoardRenderer : MonoBehaviour, IUnityGameBoardRenderer, IGameBoardDataProvider<IUnityGridSlot>
    {
        ...
        [Space]
        [SerializeField] private float _tileSize = 0.8f;
        ...
        bool IUnityGameBoardRenderer.IsPointerOnBoard(Vector3 worldPointerPosition, out GridPosition gridPosition)
        {
            return IsPointerOnBoard(worldPointerPosition, out gridPosition);
        }

        Vector3 IUnityGameBoardRenderer.GetWorldPosition(GridPosition gridPosition)
        {
        ...
        {
            var offsetY = Mathf.Floor(rowCount / 2.0f) * _tileSize - 2.0f;
        ...
        private void CreateTiles(int[,] data)
        {
            for (var rowIndex = 0; rowIndex < _rowCount; rowIndex++)
            {
                ...
                {
                    var gridTile = GetTile(rowIndex, columnIndex, (TileGroup)data[rowIndex, columnIndex]);
