...
def test_db_cache_performance():
    ...
    results1 = backtest.run(stock_pool=stock_codes, verbose=True, batch_size=20)
    ...
    results2 = backtest2.run(stock_pool=stock_codes, verbose=True, batch_size=20)
    ...
    # 显示数据库大小
    if os.path.exists('stock_data.db'):
        size = os.path.getsize('stock_data.db')
        print(f"数据库大小: {size / 1024 / 1024:.2f} MB")

if __name__ == "__main__":
    test_db_cache_performance()
