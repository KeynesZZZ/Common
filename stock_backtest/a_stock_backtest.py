...
class AStockBacktest:
    ...
    def run(self, stock_pool: Optional[List[str]] = None, verbose: bool = True) -> Dict:
        if verbose:
            print("=" * 80)
            print("A股策略回测")
            print("=" * 80)
        
        data = self._load_data(stock_pool, verbose)
        if not data:
            if verbose:
                print("未加载到任何数据，回测终止")
            return {}
    ...
    def _load_data(self, stock_pool: Optional[List[str]], verbose: bool) -> Dict:
        if verbose:
            print(f"\n加载数据...")
    ...
    def _generate_signals(self, data: Dict, verbose: bool) -> pd.DataFrame:
        ...
        strategy = LimitUpStrategy(self.strategy_params)
        signals = []
